...
assignin('base','s',draw_data)
combxTemp=comb(draw_data.x);
combyTemp=comb(draw_data.y);

%Remove NaNs
sel=~isnan(combxTemp) & ~isnan(combyTemp);
combxTemp=combxTemp(sel);
combyTemp=combyTemp(sel);

%fit to distribution
if all(combyTemp == 1)
    if params.nbins == 0
        [count,edges] = histcounts(combxTemp);
    else
        [count,edges] = histcounts(combxTemp,params.nbins);
    end
    for index = 1 : length(edges) - 1
        combx(index) = (edges(index) + edges(index + 1)) / 2;
    end
    comby = count ./ sum(count);
else
    combx = combxTemp;
    comby = combyTemp;
end
...
fo = fitoptions('Method','NonlinearLeastSquares','Startpoint',params.StartPoint);
ft = fittype(params.fun);
[mdl,gof,~]=fit(shiftdim(combx),shiftdim(comby),ft,fo);
